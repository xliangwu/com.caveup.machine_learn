...
def crawl_content(url):
    ...
    try:
        cookies={"__jsluid_h":"ec153067397a34fdc97ca710fa557df9","__jsl_clearance":"1577502063.439|0|cnvongdHKKLqNObgw0ZI9JOW3B4%3D"}
...
def crawer_all_links(url):
    ...
    all_links = []
    try:
        cookies={"__jsluid_h":"1ab28b6e9a8e92d1f9339484b5a93de2","__jsl_clearance":"1577467487.227|0|PtNp46n8hvJ9xngwfXcP9JTxrKc%3D"}
        response = requests.get(url, headers=http_header,cookies=cookies)
        print(response.status_code)
        if response.status_code != 200:
            print(url, "返回代码不是200")
...
def crawl_link_end_number(url):
    ...
    try:
        cookies={"__jsluid_h":"ec153067397a34fdc97ca710fa557df9","__jsl_clearance":"1577459316.89|0|0V1z4%2BXGKA6ojH%2F%2BG9wypRBSbOY%3D"}
        response = requests.get(url, headers=http_header,cookies=cookies)
        if response.status_code != 200:
            print(url, "返回代码不是200")
            return
...
if __name__ == '__main__':
    ...
    crawl_content("http://www.cbirc.gov.cn/beijing/ybjhDocPcjgView/05B45037F7B04151907D80DA601EE0A7/26.html")
